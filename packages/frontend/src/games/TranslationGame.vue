...
import { useQuizGame, shuffleArray, pickRandom, getDistractors } from './useQuizGame';
...
const options = computed(() => {
  if (!quiz.currentWord.value?.gameData?.translation) return [];
  const correctTranslation = quiz.currentWord.value.gameData.translation;
  let distractors = getDistractors(props.words, quiz.currentWord.value, optionCount - 1, 'translation');
  if (distractors.length < optionCount - 1 && quiz.currentWord.value.gameData) {
    const needed = optionCount - 1 - distractors.length;
    const gameDistractors = pickRandom(
      quiz.currentWord.value.gameData.distractorTranslations.filter((d: string) => !distractors.includes(d)),
      needed,
    );
...
const correctAnswer = computed(() => quiz.currentWord.value?.gameData?.translation || '');
</script>
  ...
  >
    <div class="flex-1 flex flex-col items-center justify-center p-4">
      <div class="mb-8 text-center">
        <p class="text-sm text-gray-500 mb-2">Translate to {{ targetLanguage }}</p>
